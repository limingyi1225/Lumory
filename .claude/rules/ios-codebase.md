...
- 启动**不主动请求麦克风 / 通知权限**(首次点录音时由 `AudioRecorder.startRecording()` 触发授权),只做动画预热、图片迁到 iCloud([ChronoteApp.swift:177](Chronote/ChronoteApp.swift:177) `migrateExistingImagesToiCloud`)、`WordCountBackfillService.backfillIfNeeded()`([ChronoteApp.swift:121](Chronote/ChronoteApp.swift:121))。
...
## 场景切换(`onChange(of: scenePhase)` 起 [ChronoteApp.swift:357](Chronote/ChronoteApp.swift:357),handler 起 [ChronoteApp.swift:380](Chronote/ChronoteApp.swift:380))

- `.background`:flush `viewContext` 防输入丢失 + `EntryDeletionUndoService.shared.commitPendingNow()` 兜底防 attachment 孤儿。
- `.active`:`syncMonitor.checkCloudKitStatus()`([ChronoteApp.swift:404](Chronote/ChronoteApp.swift:404))+ `ReminderService.shared.requestReschedule()`([ChronoteApp.swift:408](Chronote/ChronoteApp.swift:408),内部已对 `!isEnabled` 早返,只 cancel pending 不跑 CoreData fetch)。
...
- [OpenAIService.swift](Chronote/Services/OpenAIService.swift) + [Services/OpenAI/](Chronote/Services/OpenAI/) — 生产实现,`.shared` singleton,走后端代理。**类名是历史遗留**(2026-06-11 GPT→Claude 迁移后主力通路是 Anthropic,改名牵 22+ callsite 不动,同 `Chronote/` 目录逻辑)。8 文件:`OpenAIService.swift`(类壳 / debouncedRequest)+ `OpenAI/SSEParser.swift / OpenAIService+Anthropic.swift / +Streaming.swift / +OneShotAI.swift / +Suggestions.swift / +ThemeAlias.swift / +Import.swift`。**`+Anthropic.swift` 是 Claude 通路底座**:`ClaudeModel.opus/.sonnet` 常量、`claudeChat`(吞错)/ `claudeChatThrowing`(typed error)、`makeAnthropicRequest`(JSONSerialization 拼 body,`output_config.format` json_schema / `thinking` adaptive)、`AnthropicStreamChunk`(SSE 帧,`message_stop` 是成功终止帧,截断信号在 `message_delta.stop_reason`)。流式底座 `streamClaudeChatEvents`(`+Streaming.swift` fileprivate,叙事 + AskPast 共用,truncatedReason 由 caller 传)。旧 `chat`/`chatThrowing`(OpenAI chat/completions)已删,`embed()` 是唯一还打 OpenAI 的调用。**2026-05-16 testability seam 不变**:`init(session:appSharedSecret:)` + MockURLProtocol;empty-secret guard 统一 `BackendErrorMapper.missingSharedSecretError()`(domain="BackendError" code=401)。`OpenAIService+Import.swift` 的 `encodeImportPayload(rawText:today:calendar:)` 强制 `Calendar(identifier: .gregorian)` + 继承 caller `timeZone` + POSIX locale —— `client_today` / `client_year` 必须同 timeZone,避免纽约 23:00 = UTC 跨日的 5/16 误归本年。
- [OnDeviceAIService.swift](Chronote/Services/OnDeviceAIService.swift) — **端上 AI(Apple Foundation Models,2026-06-11 新增)**。enum + static 方法:`moodScore` / `themes` / `summary` / `suggestions`,`@Generable` 守护式生成(`MoodScoreResult` 等),每次调用新建 `LanguageModelSession`(单轮无状态,指令放 `instructions`、日记原文放 prompt 做注入隔离)。**降级契约:返回 nil = 端上不可用或生成失败,caller(OneShotAI / Suggestions)回落云端 Sonnet 4.6**;触发 nil:`SystemLanguageModel.default.availability != .available` / 输入超 `maxInputUTF16Units`(6000,超长不截断直接走云端)/ guardrails 拒答(日记心理健康内容可能误伤)/ 其他生成错误。标签清洗(`cleanThemeTags`)/ 范围校验在 caller 跟云端共用。
...
- [ThemeAliasJudgeService.swift](Chronote/Services/ThemeAliasJudgeService.swift) — 把 "AI 判断 → 入待审队列"封起来。两条入口:`judgeAfterWrite(entryID:newTags:)`(写日记后调,**60s soft throttle**)/ `scanAllHistory()`(Settings "扫描已有主题",**模型 claude-opus-4-8 + adaptive thinking + effort=medium**)。两条都不阻塞,失败静默。
- [InsightsEngine.swift](Chronote/Services/InsightsEngine.swift) — Insights / Ask-Your-Past / 写作伙伴 统一聚合入口,`performBackgroundTask` 读 CoreData,只返值类型。**2026-05-16 round 3 拆 facade**:`searchSemantic` / `ask` 实现搬到 `InsightsSearchEngine.swift`(下一行),本文件留 facade forwarder(各一行 forward)+ aggregator(`aggregateMoodSeries` / `computeStreaks` / `aggregateThemes` / `streamNarrativeEvents` / `recentEntries`)+ range counters + 嵌套类型(`SemanticSearchResult` / `AnswerChunk` 留这里因为 view 层 caller 用,搬走会拖 4 个 view + 17 处 test 引用)+ static forwarding wrapper(`cosineSimilarity` / `rankRetrieval` 各一行 forward,留兼容 ChronoteTests.swift 15 处静态调用)。~557 行(拆完时 470,后续业务增长,行数会漂)。**关键不变量**:`init` 内 `self.searchEngine = InsightsSearchEngine(persistence: persistence, ai: ai)` 保跟主 engine 同生命周期 + 同注入(测试零改动),SearchEngine 类型本身**不**是 singleton。
- [InsightsSearchEngine.swift](Chronote/Services/InsightsSearchEngine.swift) — **Search + RAG 子系统**(2026-05-16 round 3 从 InsightsEngine 拆出)。~485 行(拆完时 368,后续业务增长,行数会漂)。持有跟 InsightsEngine 同一套 `persistence` + `ai` 注入。包含 `searchSemantic` / `ask` instance method + `retrieve` / `materialize` private + `rankRetrieval` / `cosineSimilarity` static helper。**反向依赖 0**:不调用 `InsightsEngine.cosineSimilarity` / `.rankRetrieval`(改用 `Self.cosineSimilarity`),只用 `InsightsEngine.SemanticSearchResult` / `.AnswerChunk` 嵌套类型 qualifier 引用(16 处)— 这是 Phase 3.1 acceptable 抽象泄漏,Phase 3.2 之前应抽 `InsightsTypes.swift` 顶层 + typealias 保 callsite 零改。Log prefix `[InsightsSearchEngine]`(不是 `[InsightsEngine]`),`category: .ai`。
...
- [NarrativeGenerationCoordinator.swift](Chronote/Services/NarrativeGenerationCoordinator.swift) — `@MainActor @Observable` 单例,管 per-range narrative 生成生命周期。两套独立状态:`generating: Set<TimeRange>`(用户主动 start/重试)+ `precomputing: Set<TimeRange>`(`NarrativePrecomputeService` 后台调 `beginPrecompute/endPrecompute` 透出来的信号)。卡片把两者并集投到 `isStreaming` 渲染 shimmer,而卡片(`NarrativeSummaryCard`)的 **`blocksUserTap` 只读 coordinator 的 `generating`**(precompute 是 silent 后台,屏蔽 tap 会让用户莫名点不动)—— `blocksUserTap` 是 view 层 computed property,不是 coordinator 自己的属性。per-range `streamGeneration` 世代号防 supersession。`cancelAll() async` 是清缓存 / 删日记入口的兜底,**清 generating + precomputing + tasks 三件套**。
- [NarrativePrecomputeService.swift](Chronote/Services/NarrativePrecomputeService.swift) — `actor`,写日记后 **8s** debounce 在后台预生成 `.month` narrative(**2026-05-28 60s → 8s**:原 60s 主要在空等 `performAIWriteback` 把 `entry.summary` 回写落地,因为旧版 narrative prompt 含「摘要」字段;删掉 prompt 里的摘要后〔见 `OpenAIService.narrativeEntryBlock`,现在只拼 date/mood/text〕narrative 只依赖创建即存在的字段,不必再等回写,debounce 退化成纯 settle 窗口故收短)。debounce 用 `Task.detached(.background)`(`Task { }` 在 actor 内不继承 isolation,显式 detached 让"不抢主线程优先级 + 非 structured" 两层意图明确)。**立刻** `beginPrecompute(.month)` 给 UI 占位,不等 8s + 30s stream 走完;`endPrecomputingIfCurrent` 走 generation 守卫确保 supersede 老 task 不清掉新 task 的 shimmer。`cancelPendingAndBumpGeneration()` 兜底清 `.month` precomputing。
...
- `Settings/` — `AdvancedSettingsView` · `SettingsBackfillRows`(`@MainActor BackfillCoordinator` + `OneClickRebuildRow` / `EmbeddingBackfillRow` / `ThemeBackfillRow`)· `SettingsEntryDeletionService`(`@MainActor enum`,`deleteAll(viewContext:) async -> Bool` —— 不接受 caller 传 entries,方法内部自己 re-fetch 全表〔2026-05-13 superreview round 2 的 race 防御〕,内含五件套 + 私有 `EntryAttachmentSnapshot: Sendable`)。
...
- `Utils/` — `AnimationConfig`(token 详见 `views-design-tokens.md` rule)/ `LiquidGlass`(含 `LumoryCornerRadius { card 16 / nestedRow 14 / chip 22 / inline 12 }`)/ `Log` / `MoodLabels` / `PerformanceOptimization` / `LumoryDateFormatters`(11 个 system-locale 共享 `DateFormatter` + 7 个 language-aware accessor;**3 个 POSIX-locked token**:`fileTimestamp`(文件名)/ `isoDatePOSIX`(LLM prompt 防本地数字系)/ `httpDate`(RFC 7231 Retry-After);收编范围已从 view 层扩到 service 层,DiaryExportService / NetworkRetryHelper / OpenAIService+Streaming 都用共享 cache,**仍保留 inline 的**只剩 prompt-specific 中文格式 / ISO 周指纹 / ISO8601DateFormatter)。`LocalizationHelper` 在 `LumoryWidgetShared/`。

## Widget(`LumoryWidgets/` + `LumoryWidgetShared/`)

- `LumoryWidgetShared/` — **双 target 共享 sources**(同时编进 Lumory 主 App + LumoryWidgets extension),**只 import Foundation**。文件:`AppGroupConstants` / `LocalizationHelper` / `LumoryWidgetKind` / `WidgetSnapshot`(Codable DTO)/ `WidgetSnapshotStore`(纯文件 IO,App Group 容器内 `Library/Application Support/Widget/snapshot.json`,`overrideURL` testing seam)/ `WidgetTodayContext`(纯函数,算 `wroteToday` + `effectiveStreak`,给 widget provider 和单测共用)。**禁止**引 UIKit / CoreData / WidgetKit / AppSecrets / 主 App service —— 一污染就编不过。
...
- **默认 true 的 `Bool` UserDefaults 不能用 `defaults.bool(forKey:)`** —— 缺省返 false,与"显式关"撞。用 `(defaults.object(forKey: key) as? Bool) ?? true`。**注意**:若场景是"默认值翻转 / 隐私 opt-in"别用 `?? true`,要走下一条的 sentinel migration —— `ReminderService.useContextualBody` 现已是默认 **false** 的隐私 opt-in,正是用 sentinel(见 `loadUseContextualBody`)而非 `?? true` 的实例,不要再拿它当"默认 true"的例子。
