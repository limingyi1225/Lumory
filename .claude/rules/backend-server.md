...
部署在 `https://lumory.isaabby.com`(Cloudflare → nginx:443 → node:3000),PM2 进程管理。代码主体集中在 [index.js](server/index.js)(约 970 行量级,会随功能增长漂;Express 5 + pino + pino-http + express-rate-limit + axios + multer + dotenv)。其余:`package.json` / `package-lock.json` / `eslint.config.js`。`ecosystem.config.js`(PM2 配置:`lumory-server`,fork 模式,`max_memory_restart: 512M`)。
