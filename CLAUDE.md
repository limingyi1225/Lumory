...
- **MCP servers**(`~/.claude.json` 本项目 scope):
  - `xcodebuildmcp` — 封装 `xcodebuild` / `simctl` / UI 自动化。**优先用它的工具**而不是 Bash 跑 `xcodebuild`。坑:(1) `build_run_sim({extraArgs: [...]})` 的 extraArgs 是**编译 flag**,不是 app 启动参数 —— 塞 `-LumoryUITestSampleData YES` 会被 xcodebuild 拒。要传 app 启动参数走三步:`build_sim` → `install_app_sim({appPath: ".../Debug-iphonesimulator/Lumory.app"})` → `launch_app_sim({args: ["-LumoryUITestSampleData", "YES"]})`。`session_set_defaults({bundleId: "Mingyi.Lumory"})` 一次,后续不用再传。(2) **UI driving 工具默认不暴露 —— 老结论成立(2026-06-02 live 实测复核)**。本机 `xcodebuildmcp@latest` = 2.6.0,直接 stdio 拉起跑 `tools/list`:**默认 24 个工具,UI 这块只有 `snapshot_ui`(读,返回 element ref)+ `screenshot`,没有 tap/swipe/drag/gesture/type_text 等驱动工具**。这些工具文件在包里(`ui-automation` workflow 组)但默认不开。开法:env `XCODEBUILDMCP_ENABLED_WORKFLOWS` —— 实测 `XCODEBUILDMCP_ENABLED_WORKFLOWS=ui-automation` 后 tap/swipe/drag/gesture/long_press/touch/button/key_press/key_sequence/type_text/wait_for_ui/batch 全出现;但它是**白名单制**,只列 `ui-automation` 会把 build/simulator 组挤掉,要把需要的 workflow 一起列。没配这 env 就维持老办法:`snapshot_ui` 只读、真要驱动 UI 走 computer-use 或 ChronoteUITests。**2026-06-02 已给 Claude Code 配上**:项目根 `.mcp.json`(进 git)+ user scope(`~/.claude.json` 顶层 `mcpServers`)双写,两处都带 `XCODEBUILDMCP_ENABLED_WORKFLOWS=simulator,ui-automation,debugging,logging`(抄自 Codex `build-ios-apps` 插件的 `.mcp.json`),所以 tap/swipe/drag/gesture 等 driving 工具开箱即用。**首次启动会弹项目 `.mcp.json` 信任确认**,approve 后才进工具池。(此前本节曾长期写"在 `~/.claude.json` 本项目 scope" —— 那是漂的旧信息,实际之前只 Codex 侧有、Claude Code 这边一直没装。)(3) Sim 窗口可能投到"External Display"虚拟屏,computer-use 看不到但 `xcodebuildmcp.snapshot_ui` 走 simctl 是 headless 的不受影响。
  - `context7`(插件)— 查 SwiftUI / CoreData / CloudKit / Express 5 等官方文档时用,避免训练截止日之后的 API 漂移。
- **Skills** 在 `.claude/skills/`(共 4 个):`screenshot`(截图流水线 + 坑,"截图/上架截图"自动触发)/ `megareview`(整库 bug+优化+功能机会审计;**2026-05-28 重构为 Workflow 驱动**:薄壳层先 `AskUserQuestion` 问两题〔关注点:全面 / 纯 UIUX / 不含 FEAT / 自定义;+ 要不要扫 dead code〕+ 摸底 Bash,然后调 `Workflow({ scriptPath: ".claude/workflows/megareview.mjs", args })` —— slice 图 / 16 条核对清单 / angle 矩阵 / 三档 schema 全内置脚本,`pipeline(review→verify)` 无 barrier 边审边核 + escalation 全局 cap 动态追派 + 结构化 schema 把 findings 回脚本运行时〔消灭旧版"写文件回摘要"hack〕;workflow 出 `kept/rejected` → 主 agent 去重/终校/写报告 → codex 二审〔薄壳层,脚本内调不动 codex:rescue〕。改 angle/slice/清单去改 `.mjs`,SKILL.md 只管薄壳流程)/ `superreview`(**2026-05-28 同样重构为 Workflow 驱动**:薄壳层锁 diff range〔默认 working tree;"未 push 到 main" → `origin/main..HEAD`、"vs main" → `main..HEAD`、"跳过 codex" 等自然语言开关〕+ **按改动文件选视角**〔diff 相关判断留薄壳,不像 megareview 固定 slice 图〕,然后调 `Workflow({ scriptPath: ".claude/workflows/superreview.mjs", args })`,同 `pipeline(review→verify)` 无 barrier + escalation cap 3 + 结构化 schema;workflow 出 `kept/rejected` → 主 agent 去重/写 MD → codex diff 复审 + 终审〔薄壳层〕→ **每次强制出给非程序员看的 HTML**。改视角池/schema/清单去改 `.mjs`)/ `sync-to-mac`(双 Mac 协作 git 同步,push/pull/ship)。
