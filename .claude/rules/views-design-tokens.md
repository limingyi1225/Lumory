...
- **Settings 的 inset-grouped Form 背景必须是全高恒定的 `systemGroupedBackground`**。不要再用中途 fade 到 `Color.clear` 的渐变：row 是白色实体面，渐变淡到 sheet 纯白后，下半屏 section 会静止时融为一体；滚进有色区域边界又突然出现。根 Settings 和 Advanced 等 push 子页各自铺同一块**不透明** grouped background（同时隐藏 `scrollContentBackground`）；不要依赖 parent 在 push / pop / 滚动时透传。相同实色重复不会像 alpha 渐变那样叠暗。
