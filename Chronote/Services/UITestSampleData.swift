...
enum UITestSampleData {
    ...
    private static func wipeAllReturningIDs(in ctx: NSManagedObjectContext) -> [NSManagedObjectID] {
        if ctx.persistentStoreCoordinator?.persistentStores.allSatisfy({ $0.type == NSInMemoryStoreType }) == true {
            let request: NSFetchRequest<DiaryEntry> = DiaryEntry.fetchRequest()
            do {
                let entries = try ctx.fetch(request)
                let ids = entries.map(\.objectID)
                entries.forEach(ctx.delete)
                return ids
            } catch {
                Log.error("[UITestSampleData] in-memory wipe failed: \(error)", category: .persistence)
                return []
            }
        }
    ...
    private static func seed(in ctx: NSManagedObjectContext) {
        ...
        for entry in allEntries {
            ...
            // 真实的小时分布,不是同一时刻一柱。
            let day = cal.date(byAdding: .day, value: -entry.daysAgo, to: now) ?? now
            ...
            obj.moodValue = entry.mood
            obj.setThemes(entry.themes)
    ...
    private static func historicalEntries() -> [SampleEntry] {
    ...
    private static let entries: [SampleEntry] = [
        ...
        SampleEntry(daysAgo: 11, hour: 22, minute: 30, mood: 0.50,
          themes: ["工作"],
        ...
        SampleEntry(daysAgo: 29, hour: 22, minute: 0, mood: 0.75,
          themes: ["家人", "麻团"],
          summary: "麻团在我妈家嗷叫了一晚",
          text: "回家吃饭，妈妈非要让我把麻团带回去过夜，理由是“它一个人在家可怜”。结果它一晚上嗷叫，凌晨 3 点把我和小满都叫醒了。\n\n明天还要上班。"),

        SampleEntry(daysAgo: 31, hour: 23, minute: 30, mood: 0.80,
          themes: ["小满", "电影", "咖啡"],
          summary: "看《海街日记》，吃糖油饼",
          text: "下班后去 Felicity 看《海街日记》，散场和小满走到武林夜市。她非要吃糖油饼，我吃了半个就饱。\n\n回家路上路过 Berry Beans，关了。明天再来。"),

        SampleEntry(daysAgo: 34, hour: 22, minute: 30, mood: 0.35,
          themes: ["GRE"],
