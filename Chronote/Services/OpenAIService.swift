...
extension OpenAIService {
        ...
        **只返回 HIGH 和 MEDIUM,LOW 一律省略。** 找不到任何匹配就返回空数组。
        ...
        for entry in arr {
            ...
            let confRaw = (entry["confidence"] as? String)?
                .trimmingCharacters(in: .whitespacesAndNewlines)
                .lowercased()
            guard let confRaw, let conf = ThemeAliasJudgeMatch.Confidence(rawValue: confRaw) else { continue }
    ...
    func scanThemeAliasGroups(
        ...
        for entry in arr {
            ...
            let confRaw = (entry["confidence"] as? String)?
                .trimmingCharacters(in: .whitespacesAndNewlines)
                .lowercased()
            guard let confRaw, let conf = ThemeAliasJudgeGroup.Confidence(rawValue: confRaw) else { continue }
    ...
    private static func snippetSummary(_ raw: String, max: Int) -> String {
