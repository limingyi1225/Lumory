...
- `Chronote/`
  ...
  - `Services/` — 业务核心。关键文件:
    ...
    - [InsightsResultCache.swift](Chronote/Services/InsightsResultCache.swift) — Insights sheet 的 SWR 缓存(`@MainActor` 单例,key=`TimeRange`)。`reload()` 入口命中即 hydrate + 跳过 isLoading,后台仍跑 query 完成后覆盖,把 sheet re-open 的"骨架→数据"换成"旧数据→新数据"。**不主动失效**(reload 总会盖掉);批量删/重建统一走 `EntryWipeOrchestrator.performBulkWipeCleanup()` 清理派生缓存。
    ...
    - `*BackfillService.swift` — `WordCountBackfillService`(启动 + remote-change 自动跑)、`EmbeddingBackfillService` / `ThemeBackfillService`(**不 auto**,由用户主动触发:Settings 的「一键重建索引」统合按钮(`OneClickRebuildRow`),或单独入口 —— embedding 的「开始」按钮 [SettingsBackfillRows.swift:321](Chronote/Views/Settings/SettingsBackfillRows.swift:321),theme 的「只修有问题的」[SettingsBackfillRows.swift:403](Chronote/Views/Settings/SettingsBackfillRows.swift:403) / 「全部重抽」[SettingsBackfillRows.swift:409](Chronote/Views/Settings/SettingsBackfillRows.swift:409))。
    ...
    - [AppSecrets.swift](Chronote/Services/AppSecrets.swift) — 后端 URL + `X-App-Secret` 共享密钥。`APP_SHARED_SECRET` 走 xcconfig → Info.plist 注入,运行时从 Bundle 读;读不到 fallback 空字符串并让后端 401,**不硬编码**。
  ...
  - `Views/`
    ...
    - `Settings/` — `AdvancedSettingsView` · `SettingsBackfillRows`(`@MainActor` `BackfillCoordinator` + `OneClickRebuildRow` / `EmbeddingBackfillRow` / `ThemeBackfillRow`)· `SettingsEntryDeletionService`(`@MainActor enum`,`deleteAll(entries:viewContext:) async -> Bool`,save 成功后 await `EntryWipeOrchestrator.performBulkWipeCleanup()` + 私有 `EntryAttachmentSnapshot: Sendable`)
...
- **启动序列**(`init()` 在 [ChronoteApp.swift:35-95](Chronote/ChronoteApp.swift:35);remote-change observer 在 [:220](Chronote/ChronoteApp.swift:220)):
  ...
  - 启动不再主动请求麦克风权限;首次点录音时由 `AudioRecorder.startRecording()` 触发系统授权。启动仍会做动画预热、图片迁到 iCloud、`WordCountBackfillService.backfillIfNeeded()`。
...
- **场景切换**([ChronoteApp.swift](Chronote/ChronoteApp.swift)):background 时 flush `viewContext` 防止用户输入丢失 + commit pending delete undo + AppLock;active 时 `syncMonitor.checkCloudKitStatus()` + `ReminderService.shared.requestReschedule()`(内部已对 `!isEnabled` 早返,只 cancel pending 不跑 CoreData fetch);inactive 时为保护 app switcher 快照提前锁屏。
...
- **Xcode 项目用 PBXFileSystemSynchronizedRootGroup**(`Chronote/` / `ChronoteTests/` / `ChronoteUITests/` / `LumoryWidgets/` / `LumoryWidgetShared/`):这些目录里新加 .swift 会按 target 同步规则自动包含,**不要**为了普通源码手编 `project.pbxproj`。
...
- **Screenshot 模式下主动授权路径必须 early return**。启动已不请求麦克风权限;`AudioRecorder.startRecording()` 在 `UITestSampleData.isActive` 时直接允许,避免 Mic 弹窗盖截图。`ReminderService.enable()` 也要早返(防 toggle 被意外触发时通知权限弹窗盖住截图);任何新加的主动 `requestAuthorization` 路径都得过这道闸。
...
- **xcconfig 注入自定义 Info.plist key 的三个坑**:
  ...
  (3) `PBXFileSystemSynchronizedRootGroup` 只自动收已声明同步根目录里的源码,**根目录的 xcconfig 不会被识别**。需要手动加 `PBXFileReference`(`lastKnownFileType = text.xcconfig`)到 PBXFileReference section + 加到 root PBXGroup children + 在 project-level Debug + Release 的 XCBuildConfiguration 里写 `baseConfigurationReference = <FILE_ID>`。
...
- **`URLSession.sharedRetrySession` 只是长超时共享 session**:`NetworkRetryHelper.swift` 里统一配置 `timeoutIntervalForResource = 300s` 给长 SSE/HTTP 留余量;没有 SSL 绕过。全仓 `URLSessionDelegate` / `didReceiveChallenge` 零命中。
...
- **批量删 entry 统一走 orchestrator**:任何**一次性删 ≥1 篇 entry** 的路径(`SettingsEntryDeletionService.deleteAll` / `DatabaseRecoveryService.recreateStore` / 未来新增的 clear-and-restore)`save()` 成功后必须 `await EntryWipeOrchestrator.performBulkWipeCleanup()`。不要手写清理清单;orchestrator 现在还负责 narrative/precompute/generation cancel、streak milestone、widget snapshot、最近删除归档等派生状态。漏调 → banner / 通知 body / 别名管理页 / Insights / widget / narrative 引用已不存在的 entry。
...
- **后端 `/root/server` 不是 git working copy**,手动维护。改 devDep / dep:直接 SSH 跑 `npm uninstall <pkg>` / `npm install <pkg>@x.y.z` —— 同时同步 `package.json` + `node_modules`,不会 touch 其他字段。改 `index.js` 部署流程(凭证存在 Codex 本地 memory,不进 git):`sshpass -p $PWD ssh root@$HOST 'cp /root/server/index.js{,.bak.$(date +%Y%m%d-%H%M%S)}'` → `scp server/index.js root@$HOST:/root/server/` → `ssh root@$HOST 'cd /root/server && node --check index.js && pm2 restart lumory-server'` → `curl https://lumory.isaabby.com/health`。
...
- **Skills** 在 `.claude/skills/`:`screenshot`(截图流水线 + 坑)、`megareview`、`superreview`、`sync-to-mac`。用户说截图 / 上架截图时会自动匹配截图技能。
- **Subagent** `coredata-migration-reviewer`(`.codex/agents/` + `.claude/agents/`):**改 `.xcdatamodeld` / `DiaryEntry+Extensions.swift` / `PersistenceController.swift` / 任何动 `DiaryEntry` schema 的服务后,主动召唤它跑一遍审查**,别等出事。
