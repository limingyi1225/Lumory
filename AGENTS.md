...
- `Chronote/`
  ...
  - `Views/`
    ...
    - `Settings/` — `AdvancedSettingsView` · `SettingsBackfillRows`(`@MainActor` `BackfillCoordinator` + `OneClickRebuildRow` / `EmbeddingBackfillRow` / `ThemeBackfillRow`)· `SettingsEntryDeletionService`(`@MainActor enum`,`deleteAll(entries:viewContext:) async -> Bool`,内含"四件套" + 私有 `EntryAttachmentSnapshot: Sendable`)
...
- **批量删 entry 四件套**:任何**一次性删 ≥1 篇 entry** 的路径(`SettingsEntryDeletionService.deleteAll` / `DatabaseRecoveryService.recreateStore` / 未来新增的 clear-and-restore)`save()` 成功后必须**同时**调四个清理:`ReminderService.shared.requestReschedule()` + `ThemeAliasResolver.shared.resetForBulkEntryWipe()`(清 groups/pending/coolUntil,**保留 negativePairs**)+ `PromptSuggestionEngine.shared.clearCache()`(置 nil + cancel inflight refresh)+ `InsightsResultCache.shared.clear()`(SWR 缓存,不清会显示引用已删 entry 的旧聚合)。漏一个 → banner / 通知 body / 别名管理页 / Insights 图表引用已不存在的 entry。
...
- **后端 `/root/server` 不是 git working copy**,手动维护。改 devDep / dep:直接 SSH 跑 `npm uninstall <pkg>` / `npm install <pkg>@x.y.z` —— 同时同步 `package.json` + `node_modules`,不会 touch 其他字段。改 `index.js` 部署流程(凭证存在 `~/.Codex/.../memory/reference_lumory_server.md`,不进 git):`sshpass -p $PWD ssh root@$HOST 'cp /root/server/index.js{,.bak.$(date +%Y%m%d-%H%M%S)}'` → `scp server/index.js root@$HOST:/root/server/` → `ssh root@$HOST 'cd /root/server && node --check index.js && pm2 restart lumory-server'` → `curl https://lumory.isaabby.com/health`。
