...
public struct WidgetTodayContext: Equatable {
    ...
    public static func compute(
        ...
        if let last = snapshot.lastEntryDate {
            ...
            // **必须 `>= 0` 守卫**:lastEntryDate 落在未来(设备时钟跑前 / CK 同步带过来未来时间 entry)
            // 时 daysAgo 是负数,光 `<= 1` 会通过,把 currentStreak 错误地继承下来。
            effectiveStreak = (daysAgo >= 0 && daysAgo <= 1) ? snapshot.currentStreak : 0
        } else {
            effectiveStreak = 0
        }
