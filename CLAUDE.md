...
- **Skills** 在 `.claude/skills/`(共 2 个):`screenshot`(截图流水线 + 坑,"截图/上架截图"自动触发)/ `sync-to-mac`(双 Mac 协作 git 同步,push/pull/ship)。**`megareview` / `superreview` 已于 2026-08-11 整体移除**(skill + `.agents` symlink + `.claude/workflows/{megareview,superreview}.mjs` 全删):内置 `/code-review`、`/security-review` 和 codex 插件已覆盖这块,自维护的多 agent 编排不再值得养。要审查走 `/code-review` 或 `codex:rescue`。`.claude/workflows/` 下剩 `bugscan.mjs` / `deadcode.mjs` / `repo-scan.mjs`(untracked,已 gitignore)。
